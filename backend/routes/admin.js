...
const {getTopCategories, getTopServices, getTopUsers, getMostOrderedServices, fetchOrderQuantityByWeekday} = require('../database/admin')
...
adminRouter.get('/topCategories',async (req,res)=>{    
 ...
 adminRouter.get('/topByWeekDay',async (req,res)=>{
   const data=await fetchOrderQuantityByWeekday();
   res.status(200).json(data)
})
