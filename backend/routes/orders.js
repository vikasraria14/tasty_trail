...
const {getOrdersByUsername, getAllOrders, updateOrderStatus, getOrdersByCategory, getOrdersBySeller} = require('../database/orders')
router.get("/getOrdersBySeller", async(req, res) => {
  console.log("Request received for retrieving order list1");
  const sellerId= req.query.sellerId
  
  
  try{
    let docs = await getOrdersBySeller(sellerId)    
