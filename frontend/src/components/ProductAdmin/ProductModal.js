...
const ServiceInfoAdmin = ({
  ...
  const handleClose = async (e) => {
    let cost = currentStatus===""?serviceForm.cost:currentStatus;
    let quantity = currentQuantity===""?serviceForm.quantity:currentQuantity;
    let finalData = { ...serviceForm, cost:cost,quantity:quantity};
    ...
    <Modal className = "model-content"show={show} onHide={() => setShow(false)}>
      ...
      <Modal.Body>
        <Form onSubmit={() => {}}>
          ...
          <Form.Group controlId="currentQuantity">
            <Form.Label>Quantity :</Form.Label>
            <input type={"number"} value={currentQuantity} onChange={(e)=>{setCurrentQuantity(e.target.value)}} required/>
